...
public class Determinizer {
  ...
  public static void determinize(String inputPath, String outputPath, List<File> toInclude) throws IOException {
    ...
    try (JarFile jarFile = new JarFile(new File(inputPath)); JarOutputStream jos = new JarOutputStream(new FileOutputStream(new File(outputPath)))) {

      List<JarEntry> entries = jarFile.stream().sorted(Comparator.comparing(JarEntry::getName)).collect(Collectors.toList());
