...
public enum ForBlockOptionalMeta implements IDatatypeFor<BlockOptionalMeta> {
  ...
  public Stream<String> tabComplete(IDatatypeContext ctx) throws CommandException {
    ...
    if (!lastProperty.contains("=")) {
      ...
      return new TabCompleteHelper().append(block.getStateDefinition().getProperties().stream().map(Property::getName)).filter(prop -> !usedProps.contains(prop)).filterPrefix(lastProperty)
        .sortAlphabetically().map(prop -> prefix + prop).stream();
    }
    ...
    return new TabCompleteHelper().append(getValues(property)).filterPrefix(lastValue).sortAlphabetically().map(val -> prefix + val).stream();
  }
