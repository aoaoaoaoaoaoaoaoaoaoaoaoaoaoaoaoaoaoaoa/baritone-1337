...
public class Paginator<E> implements Helper {
  ...
  public int getMaxPage() { return (entries.size() - 1) / pageSize + 1; }
  ...
  public void display(Function<E, Component> transform, String commandPrefix) {
    ...
    if (hasPrevPage) {
      prevPageComponent.setStyle(prevPageComponent.getStyle().withClickEvent(new ClickEvent.RunCommand(String.format("%s %d", commandPrefix, page - 1)))
    ...
    if (hasNextPage) {
      nextPageComponent.setStyle(nextPageComponent.getStyle().withClickEvent(new ClickEvent.RunCommand(String.format("%s %d", commandPrefix, page + 1)))
