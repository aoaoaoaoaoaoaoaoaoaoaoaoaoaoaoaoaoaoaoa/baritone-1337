    ...
    return List.of(
      ...
      new Category("Pathing",
        List.of(entry("Chunk cache", s.chunkCaching, "Use, pack, and save simplified chunk facts."), entry("Cached-only paths", s.pathThroughCachedOnly, "Do not use live chunks as pathing facts."),
          ...
          entry("Ped hot local", s.pedestrianHotLocalValueField, "Use the pedestrian reverse local value field."),
          ...
          entry("Macro transports", s.macroPlanning, "Enable certified route-level transports such as boats and portals; value-field local exits remain core pathing."),
          ...
          entry("Value horizon", s.macroBiomeHorizonBlocks, "Search radius for rolling value-field local exits."),
          entry("Biome waypoint", s.macroBiomeWaypointBlocks, "Local waypoint distance along the macro route."),
      ...
      new Category("Render", List.of(entry("Render path", s.renderPath, "Draw current and next path."), entry("Path as line", s.renderPathAsLine, "Draw line path instead of boxes."),
        ...
        entry("Macro plan", s.renderMacroPlan, "Draw route-level macro legs."), entry("A* diagnostics", s.renderPathCalculation, "Draw volatile best-so-far and most-recent-considered search probes."),
