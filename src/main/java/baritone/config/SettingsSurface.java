  ...
  public static List<Category> categories() {
    ...
    return List.of(
      ...
      new Category("Pathing",
        List.of(entry("Chunk cache", s.chunkCaching, "Use, pack, and save simplified chunk facts."), entry("Cached-only paths", s.pathThroughCachedOnly, "Do not use live chunks as pathing facts."),
          entry("Cut at load boundary", s.cutoffAtLoadBoundary, "Trim execution paths to live chunks."), entry("Primary timeout ms", s.primaryTimeoutMS, "Normal A* success timeout."),
          ...
          entry("Early incumbents", s.pathingEarlyIncumbentExecution, "Execute stable partial A* paths before final timeout."),
          entry("Continuous planning", s.pathingContinuousPlanning, "Keep extending from the current segment tail in the background."),
          entry("Incumbent interval ms", s.pathingIncumbentIntervalMS, "Worker-thread incumbent publication cadence."),
          entry("Min incumbent length", s.pathingMinIncumbentLength, "Minimum partial path length to execute."),
          entry("Incumbent margin", s.pathingIncumbentHeuristicMargin, "Heuristic progress required to replace a plan."),
          entry("A* max nodes", s.pathingMaxNodes, "Hard node-arena memory fuse for one legacy A* query."),
          entry("Ped incumbent early", s.pedestrianPathingEarlyIncumbentExecution, "Pedestrian-specific incumbent execution."),
          entry("Ped incumbent interval", s.pedestrianPathingIncumbentIntervalMS, "Pedestrian-specific incumbent cadence."),
          entry("Ped incumbent length", s.pedestrianPathingMinIncumbentLength, "Pedestrian-specific incumbent length floor."),
          entry("Ped incumbent margin", s.pedestrianPathingIncumbentHeuristicMargin, "Pedestrian-specific incumbent replacement margin."),
          ...
          entry("Horse incumbent length", s.horsePathingMinIncumbentLength, "Mounted horse incumbent length floor."),
          entry("Horse incumbent margin", s.horsePathingIncumbentHeuristicMargin, "Mounted horse incumbent replacement margin."),
          entry("Transport debug", s.transportDebugOverlay, "Show live transport-mode executor state in the actionbar."),
          entry("Farfield", s.farfieldPlanning, "Price exact frontier exits with scalar continuation values."),
          entry("Transit", s.transitPlanning, "Enable explicit multimodal/meso-task route candidates such as portals and boats."),
          entry("Biome values", s.macroBiome, "Use empirical-biome facts and priors for rolling value-field local exits."),
          entry("Biome priors", s.macroBiomePriorsFile, "Measured biome traversal prior table."), entry("Biome facts", s.macroBiomeFactsFile, "Chunk-biome fact atlas."),
          entry("Farfield horizon", s.farfieldHorizonBlocks, "Search radius for Farfield frontier continuation."),
          entry("Farfield waypoint", s.farfieldWaypointBlocks, "Minimum distance before using a Farfield frontier objective."),
          entry("Farfield lateral", s.farfieldLateralCells, "Lateral 16x16 column padding around the Farfield corridor."),
          entry("Farfield commit ticks", s.farfieldMinCommittedPrefixTicks, "Minimum committed prefix before replacing a Farfield-advised route."),
          entry("Portal win ticks", s.macroPortalMinImprovementTicks, "Absolute portal-over-direct macro margin."),
          entry("Portal win ratio", s.macroPortalMinImprovementRatio, "Relative portal-over-direct macro margin."),
          entry("Farfield open tpb", s.farfieldOpenSurfaceTicksPerBlock, "Expected ticks per block for supported open continuation."),
          entry("Farfield void tpb", s.farfieldVoidTicksPerBlock, "Expected ticks per block for void/gap continuation."),
          entry("Farfield lava tpb", s.farfieldLavaTicksPerBlock, "Expected ticks per block for lava bridging/detour continuation."),
          entry("Farfield solid tpb", s.farfieldSolidTicksPerBlock, "Expected ticks per block for blind solid tunneling continuation."),
          entry("Farfield unknown tpb", s.farfieldUnknownTicksPerBlock, "Expected ticks per block outside the resident Farfield envelope."),
          entry("Farfield vertical up", s.farfieldVerticalUpTicksPerBlock, "Expected ticks per vertical block for climbing between Farfield strata."),
          entry("Farfield vertical down", s.farfieldVerticalDownTicksPerBlock, "Expected ticks per vertical block for descending between Farfield strata."),
          entry("End void prior", s.farfieldEndUnknownVoidProbability, "Unknown End column void/gap prior."),
          entry("Nether lava prior", s.farfieldNetherUnknownLavaProbability, "Unknown middle-Nether stratum lava prior."),
          entry("Nether solid prior", s.farfieldNetherUnknownSolidProbability, "Unknown middle-Nether stratum solid prior."),
          entry("Macro water horizon", s.macroWaterHorizonBlocks, "Atlas radius for water macro planning."),
          entry("Macro water launches", s.macroWaterMaxLaunchCandidates, "Maximum water launch candidates."),
          entry("Macro segment max", s.macroWaterMaxSegmentBlocks, "Maximum straight macro water segment length."),
          entry("Boat depth guard", s.macroBoatRequiresWaterBelow, "Require water below boat macro cells."))),
      new Category("Render",
        List.of(entry("Render path", s.renderPath, "Draw current and next path."), entry("Path as line", s.renderPathAsLine, "Draw line path instead of boxes."),
          ...
          entry("Farfield", s.renderFarfield, "Draw Farfield continuation hints."), entry("Transit", s.renderTransit, "Draw explicit Transit candidates and anchors."),
          entry("Macro plan", s.renderMacroPlan, "Draw legacy route-level macro legs."),
          entry("A* diagnostics", s.renderPathCalculation, "Draw volatile best-so-far and most-recent-considered search probes."),
          entry("Macro anchors", s.renderMacroPlanAnchors, "Draw macro launch, landing, and terminal anchors."), entry("Macro live color", s.colorMacroLivePlan, "Macro cells grounded in live facts."),
          entry("Macro cache color", s.colorMacroCachedPlan, "Macro cells grounded in cached facts."),
          entry("Macro predicted color", s.colorMacroPredictedPlan, "Macro cells grounded in generator predictions."),
          entry("Macro prior color", s.colorMacroPriorPlan, "Macro cells using only priors."), entry("Render cache", s.renderCachedChunks, "Draw cached chunks."),
          entry("Cache opacity", s.cachedChunksOpacity, "Opacity for cached chunk rendering."), entry("Fade path", s.fadePath, "Fade distant path segments."),
          entry("Path line width", s.pathRenderLineWidthPixels, "Path renderer line width."), entry("Goal line width", s.goalRenderLineWidthPixels, "Goal renderer line width."))),
