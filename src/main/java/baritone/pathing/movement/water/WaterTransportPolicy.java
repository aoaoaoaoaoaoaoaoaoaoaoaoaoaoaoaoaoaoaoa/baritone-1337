...
public record WaterTransportPolicy(boolean boatAvailable, boolean boatMounted, int boatMinimumBlocks, double boatSetupCost, double boatPickupCost, double boatCostPerBlock, double boatHeadingX,
  ...
  private static final double BOAT_PICKUP_COST = 35D;
  ...
  public static WaterTransportPolicy snapshot(Baritone baritone) {
    ...
    if (vehicle instanceof AbstractBoat boat) {
      ...
      if (speed > 0.05D) {
        return new WaterTransportPolicy(true, true, BOAT_MINIMUM_BLOCKS, BOAT_SETUP_COST, BOAT_PICKUP_COST, BOAT_COST_PER_BLOCK, vx / speed, vz / speed);
      }
      return new WaterTransportPolicy(true, true, BOAT_MINIMUM_BLOCKS, BOAT_SETUP_COST, BOAT_PICKUP_COST, BOAT_COST_PER_BLOCK, 0D, 0D);
    }
    return new WaterTransportPolicy(baritone.getInventoryBehavior().hasBoat(), false, BOAT_MINIMUM_BLOCKS, BOAT_SETUP_COST, BOAT_PICKUP_COST, BOAT_COST_PER_BLOCK, 0D, 0D);
  }
