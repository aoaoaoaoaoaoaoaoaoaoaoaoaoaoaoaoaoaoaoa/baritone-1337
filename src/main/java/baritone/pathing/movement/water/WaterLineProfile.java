...
public record WaterLineProfile(SurfaceWaterMode mode, double halfWidth, int minimumBlocks, double setupCost, double pickupCost, double costPerBlock) {
  public static final double SWIM_HALF_WIDTH = 0.30000001192092896D;
  public static final double BOAT_HALF_WIDTH = 0.49D;
  ...
  public static WaterLineProfile boatLaunch(WaterTransportPolicy policy, boolean terminal) {
    return new WaterLineProfile(new SurfaceWaterMode.Boat(), BOAT_HALF_WIDTH, policy.boatMinimumBlocks(), policy.boatSetupCost(), terminal ? policy.boatPickupCost() : 0D, policy.boatCostPerBlock());
  }
