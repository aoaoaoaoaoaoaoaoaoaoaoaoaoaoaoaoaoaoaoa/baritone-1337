...
public class MovementDiagonal extends Movement {
  ...
  public MovementDiagonal(IBaritone baritone, BetterBlockPos start, Direction dir1, Direction dir2, int dy) {
    ...
    // super(start, start.offset(dir1).offset(dir2), new BlockPos[]{start.offset(dir1), start.offset(dir1).up(), start.offset(dir2), start.offset(dir2).up(), start.offset(dir1).offset(dir2),
    // start.offset(dir1).offset(dir2).up()}, new BlockPos[]{start.offset(dir1).offset(dir2).down()});
  }
  ...
  public static void cost(CalculationContext context, int x, int y, int z, int destX, int destZ, EdgeEvalScratch res) {
    ...
    if (MovementHelper.isWater(startState) || MovementHelper.isWater(destInto)) {
      ...
      multiplier = context.costs.waterMoveCost();
      water = true;
    }
