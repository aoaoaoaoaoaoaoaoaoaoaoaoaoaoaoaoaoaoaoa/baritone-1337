 ...
 * @author leijurv
 */
public class RouteExecutor implements IPathExecutor, Helper {
  ...
  private final List<SurfaceLineLeg> surfaceLegs;
  private SurfaceLineLeg activeSurfaceLeg;
  ...
  public RouteExecutor(PathingBehavior behavior, IPath path) {
    ...
    this.surfaceLegs = surfaceLegs(behavior, path);
    this.pathPosition = 0;
  }
  ...
  public boolean onTick() {
    ...
    for (int revisions = 0; revisions < MAX_POSITION_REVISIONS_PER_TICK; revisions++) {
      ...
      SurfaceLineLeg surfaceLeg = surfaceLegAt(pathPosition);
      if (surfaceLeg != null) {
  ...
  private boolean surfaceLineSprintRequested() {
    boolean requested = controlFrame.input(Input.SPRINT);
    controlFrame = controlFrame.mutate().setInput(Input.SPRINT, false).build();
    return requested && new CalculationContext(behavior.baritone, false).movement.canSprint();
  }
  ...
  private static List<SurfaceLineLeg> surfaceLegs(PathingBehavior behavior, IPath path) {
  ...
  private SurfaceLineLeg surfaceLegAt(int position) {
    for (SurfaceLineLeg leg : surfaceLegs) {
  ...
  private RouteExecutor cutIfTooLong() {
    ExecutionPolicy policy = ExecutionPolicy.capture(behavior);
  ...
  private RouteExecutor transplant(IPath path, int pathPosition, Integer costEstimateIndex) {
    RouteExecutor ret = new RouteExecutor(behavior, path);
