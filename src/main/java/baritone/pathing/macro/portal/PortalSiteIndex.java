...
final class PortalSiteIndex {
  ...
  private PortalSiteIndex(Long2ObjectOpenHashMap<ArrayList<PortalSite>> bySourceCell) {
  ...
  static PortalSiteIndex build(MacroExpansionContext context) {
    ...
    addCachedPortals(context, byCell, siteKeys);
    addCachedObsidian(context, obsidian);
    addLiveBlocks(context, byCell, siteKeys, obsidian);
    ...
    return new PortalSiteIndex(byCell);
  }
  ...
  private static void addCachedPortals(MacroExpansionContext context, Long2ObjectOpenHashMap<ArrayList<PortalSite>> byCell, LongOpenHashSet siteKeys) {
    ...
    for (BlockPos pos : calculation.worldData.getCachedWorld().getLocationsOf("nether_portal", MAX_SITES, context.physicalStart().x, context.physicalStart().z, regionRadius * regionRadius)) {
      if (flatDistance(context.physicalStart(), pos) <= radius) {
        addSite(context, byCell, siteKeys, PortalSiteKind.LIT_PORTAL, new BetterBlockPos(pos), 0);
      }
    }
  }
  ...
  private static void addLiveBlocks(MacroExpansionContext context, Long2ObjectOpenHashMap<ArrayList<PortalSite>> byCell, LongOpenHashSet siteKeys, ArrayList<BetterBlockPos> obsidian) {
    ...
    for (int cx = minCx; cx <= maxCx && siteKeys.size() < MAX_SITES; cx++) {
      for (int cz = minCz; cz <= maxCz && siteKeys.size() < MAX_SITES; cz++) {
        ...
        scanLiveChunk(context, byCell, siteKeys, obsidian, calculation.world.getChunkSource().getChunk(cx, cz, false), minX, maxX, minY, maxY, minZ, maxZ, start, radiusSq);
      }
    }
  }

  private static void scanLiveChunk(MacroExpansionContext context, Long2ObjectOpenHashMap<ArrayList<PortalSite>> byCell, LongOpenHashSet siteKeys, ArrayList<BetterBlockPos> obsidian, LevelChunk chunk,
    int minX, int maxX, int minY, int maxY, int minZ, int maxZ, BetterBlockPos start, int radiusSq) {
    ...
    for (int sectionIndex = 0; sectionIndex < sections.length && siteKeys.size() < MAX_SITES; sectionIndex++) {
      ...
      scanLiveSection(context, byCell, siteKeys, obsidian, chunk, section, sectionY, minX, maxX, minY, maxY, minZ, maxZ, start, radiusSq);
    }
  }
  ...
  private static void scanLiveSection(MacroExpansionContext context, Long2ObjectOpenHashMap<ArrayList<PortalSite>> byCell, LongOpenHashSet siteKeys, ArrayList<BetterBlockPos> obsidian,
    LevelChunk chunk, LevelChunkSection section, int sectionY, int minX, int maxX, int minY, int maxY, int minZ, int maxZ, BetterBlockPos start, int radiusSq) {
      ...
      for (int z = fromZ; z <= toZ && siteKeys.size() < MAX_SITES; z++) {
        ...
        for (int x = fromX; x <= toX && siteKeys.size() < MAX_SITES; x++) {
          ...
          if (state.is(Blocks.NETHER_PORTAL)) {
            addSite(context, byCell, siteKeys, PortalSiteKind.LIT_PORTAL, new BetterBlockPos(worldX, worldY, worldZ), 0);
    ...
    for (BetterBlockPos block : obsidian) {
      for (Direction.Axis axis : List.of(Direction.Axis.X, Direction.Axis.Z)) {
        for (BetterBlockPos lowerLeft : PortalFrame.candidateLowerLefts(block, axis)) {
          ...
          addSite(context, byCell, siteKeys, frame.kind(), PortalFrame.center(frame), frame.missingObsidian());
        }
      }
    }
  }
  ...
  private static void addSite(MacroExpansionContext context, Long2ObjectOpenHashMap<ArrayList<PortalSite>> byCell, LongOpenHashSet siteKeys, PortalSiteKind kind, BetterBlockPos interaction,
    int missingObsidian) {
    ...
    BetterBlockPos target = PortalGeometry.transform(interaction, dimensionId, targetDimensionId);
    long targetCell = MacroNodeKey.cell(targetDimensionId, MacroStratum.SURFACE, scale, Math.floorDiv(target.x, cellBlocks), Math.floorDiv(target.z, cellBlocks));
    long siteKey = siteKey(sourceCell, interaction, missingObsidian, kind);
    if (!siteKeys.add(siteKey)) {
      return;
    }
    byCell.computeIfAbsent(sourceCell, ignored -> new ArrayList<>()).add(new PortalSite(kind, interaction, target, sourceCell, targetCell, dimensionId, targetDimensionId, missingObsidian));
  }
