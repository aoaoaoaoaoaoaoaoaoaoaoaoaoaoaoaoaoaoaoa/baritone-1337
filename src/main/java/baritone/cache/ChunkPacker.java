...
public final class ChunkPacker {
        ...
        for (int y1 = 0; y1 < 16; y1++) {
          ...
          for (int z = 0; z < 16; z++) {
            for (int x = 0; x < 16; x++) {
              ...
              getPathingBlockType(state, chunk, x, y + chunk.getMinY(), z).writeTo(bitSet, index);
  ...
  private static PathingBlockType getPathingBlockType(BlockState state, LevelChunk chunk, int x, int y, int z) {
    ...
    if (MovementHelper.isWater(state)) {
      ...
      if ((x != 15 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x + 1, adjY, z))) || (x != 0 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x - 1, adjY, z)))
        || (z != 15 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x, adjY, z + 1))) || (z != 0 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x, adjY, z - 1)))) {
        return PathingBlockType.AVOID;
      }
